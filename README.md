...
- локальный файл из «открыть файл…» и файл вне корня тоже получают внутреннюю вкладку: возврат по клику перечитывает дескриптор (или File) и его содержимое; список вкладок живёт в текущей вкладке браузера и не переносится в новые — после F5 остаётся только открытый файл
