...
- локальный файл из «открыть файл…» и файл вне корня тоже получают внутреннюю вкладку: возврат по клику перечитывает дескриптор (или File); список вкладок и активная переживают F5 (он привязан к вкладке браузера через window.name и не переносится в новые окна; локальные файлы после F5 подхватываются по дескриптору)
