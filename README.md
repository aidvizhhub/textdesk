...
- вкладки: полоса открытых файлов под тулбаром — клик открывает в текущей вкладке браузера, крестик и средний клик закрывают, активная подсвечена фоном холста; при закрытии активной открывается соседняя, а последняя возвращает пустую рабочую зону
- локальный файл из «открыть файл…» тоже получает внутреннюю вкладку, и она переживает F5: дескриптор лежит в IndexedDB под ключом вкладки, после перезагрузки клик по вкладке просит доступ (чтение) и открывает файл; закрыл вкладку — дескриптор стёрт. список вкладок и активная привязаны к вкладке браузера через window.name и не переносятся в новые окна; фолбэк без File System Access API (File в памяти) не сохраняется
