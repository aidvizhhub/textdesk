...
fileInput.addEventListener('change', async () => {
  ...
  if(!f) return;
  if(!confirmLeave()){ fileInput.value = ''; return; }
  ...
  if(!rel){
    target.textContent = !serverMode ? 'без сервера правки писать некуда' :
      matches.length > 1 ? 'таких файлов несколько в корне — выбери по имени в «файлах» (Ctrl+P)' : 'файл вне папки сервера — писать некуда';
