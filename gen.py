...
const tabsFromStore = Array.isArray(st.tabs) ? st.tabs.slice(-12) : [];   // снимок вкладок до первого save(), иначе гонка с ?file= их затрёт
...
function tabOpen(rel, name){
  if(!rel) return;
  tabRestore(serverFiles.length ? serverFiles : null);   // вкладки прошлой сессии раньше новой
...
function tabRestore(files){
  if(!tabsFromStore.length) return;
  for(const rel of tabsFromStore){
    if((!files || files.indexOf(rel) !== -1) && !tabs.some(t => t.rel === rel)) tabs.push({rel, name: rel.split('/').pop()});
  }
  tabsFromStore.length = 0;
  tabRender();
}
