...
let curName = '';
let mediaOn = false, mediaZ = 1, mediaX = 0, mediaY = 0, mediaURL = null, mediaDrag = null;
...
function upd(){
  ...
  const total = v.split('\n').length;
  if(mediaOn){
    pos.textContent = isVideoName(curName) ? 'видео' : 'картинка';
    cnt.textContent = '';
...
let backStale = false;
function buildBack(){
  if(document.body.classList.contains('preview') || mediaOn){ backStale = true; return; }   // холст скрыт, ширина textarea = 0
...
document.querySelector('.editor').addEventListener('wheel', e => {
  if(e.ctrlKey) return;
  if(mediaOn) return;   // колесо над картинкой зумит, а не мотает скрытый текст
...
function targetText(){
  ...
  if(mediaOn) return 'только просмотр: ' + (isVideoName(curName) ? 'видео' : 'картинка') + ' — колесо зумит, двойной клик вписывает';
...
function setPreview(on){
  ...
  if(can){
    ...
    const dir = serverTarget ? '/' + (serverTarget.includes('/') ? serverTarget.split('/').slice(0, -1).join('/') + '/' : '') : '';
    if(dir) for(const el of view.querySelectorAll('img[src],a[href]')){
